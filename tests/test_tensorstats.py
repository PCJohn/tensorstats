...
class TestCorrectness:
    ...
    def test_per_channel_shape_and_values(self):
        ...
        np.testing.assert_allclose(out["0,1"], numpy_moments(arr, axes=(0, 1)), rtol=1e-8)
    ...
    def _numpy_grid_ref(self, arr, grid):
        ...
        def recurse(d, cell_coords, slices):
            if d == ndim:
                ...
                result[tuple(cell_coords)] = [mu, d2.mean(), (d2*dv).mean(), (d2*d2).mean()]
                return
            n = arr.shape[d]; nc = n_cells[d]
            for ci in range(nc):
                lo = ci * n // nc; hi = (ci+1) * n // nc
...
class TestStride:
    ...
    def test_stride_accuracy_vs_numpy(self, stride):
        ...
        mu  = sub.mean(); d = sub - mu; d2 = d*d
        ref = np.array([mu, d2.mean(), (d2*d).mean(), (d2*d2).mean()])
        errs = np.abs(out["global"] - ref) / (np.abs(ref) + 1e-10)
        names = ["mean", "var", "m3", "m4"]
        print(f"\n  stride={stride}:  " + "  ".join(f"{n}_err={e:.2e}" for n,e in zip(names,errs)))
...
class TestUint8:
    ...
    CASES = {
        ...
        "gaussian_blob":     lambda rng: np.clip(rng.normal(128, 30, (64, 64, 3)), 0, 255).astype(np.uint8),
        "horizontal_gradient": lambda rng: np.tile(np.arange(64, dtype=np.uint8).reshape(1, 64, 1), (64, 1, 3)),
        "two_tone":          lambda rng: (rng.integers(0, 2, (64, 64, 3)) * 255).astype(np.uint8),
        "near_constant":     lambda rng: np.clip(rng.integers(127, 130, (64, 64, 3)), 0, 255).astype(np.uint8),
        "checkerboard":      lambda rng: (((np.arange(64).reshape(-1,1) + np.arange(64).reshape(1,-1)) % 2)[:,:,None] * np.ones((1,1,3))).astype(np.uint8) * 255,
    }
    ...
    def test_per_channel_matches_numpy_all_cases(self, name):
        ...
        np.testing.assert_allclose(out["0,1"], ref, rtol=1e-5,
                                   err_msg=f"case={name}")
    ...
    def test_stride_accuracy_uint8(self, stride):
        ...
        errs = np.abs(out["global"] - ref) / (np.abs(ref) + 1e-10)
        names = ["mean", "var", "m3", "m4"]
        print(f"\n  uint8 stride={stride}:  " + "  ".join(f"{n}_err={e:.2e}" for n,e in zip(names,errs)))
...
class TestLatency:
    ...
    def test_statscomputer_faster_than_fresh(self, shape):
        ...
        fresh_ms    = timeit_ms(lambda: ts.StatsComputer(
            shape=shape, axes=[None,(0,1)], grid=(3,3,0)).compute(arr))
